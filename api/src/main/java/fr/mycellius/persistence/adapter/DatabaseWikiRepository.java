...
public class DatabaseWikiRepository implements WikiRepository {
    ...
    public DatabaseWikiRepository(
            WikiPageJpaRepository pageJpa,
    ...
    public WikiPage save(WikiPage page) {
        ...
        // Date de création définie si elle est absente
        if (entity.getCreatedAt() == null) {
            entity.setCreatedAt(Instant.now());
        }

        // Tags : recherche ou création avec normalisation
    ...
    public Page<WikiPage> findAll(Pageable pageable) {
        return pageJpa.findAll(pageable)
                .map(mapper::toDomain);
    }

    @Override
    public Page<WikiPage> findAllPublic(Pageable pageable) {
        return pageJpa.findByConfidentialFalse(pageable)
                .map(mapper::toDomain);
    }
    ...
    public Page<WikiPage> searchByTitle(
            String title,
            Pageable pageable) {
        return pageJpa.findByTitleContainingIgnoreCase(title, pageable)
                .map(mapper::toDomain);
    }

    @Override
    public Page<WikiPage> searchPublicByTitle(
            String title,
            Pageable pageable) {
        return pageJpa
                .findByTitleContainingIgnoreCaseAndConfidentialFalse(
                        title,
                        pageable)
                .map(mapper::toDomain);
    }
