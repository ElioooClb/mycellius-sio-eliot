...
public class InMemoryWiki implements WikiRepository {
    ...
    public Page<WikiPage> findAll(Pageable pageable) {
        ...
        all.sort(Comparator.comparing(WikiPage::getId));
    ...
    public Page<WikiPage> findAllPublic(Pageable pageable) {
        List<WikiPage> publicPages = store.values()
                .stream()
                .filter(page -> !page.isConfidential())
                .sorted(Comparator.comparing(WikiPage::getId))
                .toList();

        return toPage(publicPages, pageable);
    }
    ...
    public Page<WikiPage> searchByTitle(
        ...
        List<WikiPage> filtered = store.values()
                .stream()
                .filter(page -> page.getTitle() != null
                        && page.getTitle()
                                .toLowerCase()
                                .contains(needle))
                .sorted(Comparator.comparing(WikiPage::getId))
    ...
    private Page<WikiPage> toPage(
            List<WikiPage> source,
            Pageable pageable) {
        ...
        List<WikiPage> content = (start >= source.size())
                ? List.of()
